...
def drive_image_with_face_attrs(source, face_attrs_h5_path, window_idx=0, max_len=None):
    ...
    for frame_idx in tqdm(range(num_frames), desc="Generating frames"):
        ...
        if frame_idx == 0:
            ...
            img = inferer.forward(source, identity_img, crop=False, smooth_pose=False,
                                target_theta=True, mix=True, mix_old=False, modnet_mask=False,
                                frame_idx=frame_idx)
            all_srs.append(source)
        else:
            # For subsequent frames, we need to create a "driver" that encodes the expression
            # The key is to pass different expressions for each frame

            # Create a driver image that will make the model extract the right expression
            # We use identity_img but the model will use our custom embeddings

            # Set the expression embedding for this frame
            if expression_embeds is not None and frame_idx < len(expression_embeds):
                # Directly set the expression embed in the model
                inferer.model.expression_embedder_nw.expression_embed = expression_embeds[frame_idx:frame_idx+1]

            # Use the theta for this frame if available
            driver_theta = None
            if thetas is not None and frame_idx < len(thetas):
                driver_theta = thetas[frame_idx]

            # Generate frame with expression from cache
            img = inferer.forward(None, identity_img, crop=False, smooth_pose=False,
                                target_theta=True, mix=True, mix_old=True,
                                modnet_mask=False, frame_idx=frame_idx)
